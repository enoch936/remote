...
public class TunnelTester {
    ...
    public ConnectionReport run(VpnStatus vpn, String gatewayVpnIp,
        ...
        checks.add(gatewayReachableCheck(gatewayVpnIp));
        checks.add(dnsCheck());
    ...
    private ConnectionReport.Check gatewayReachableCheck(String gatewayVpnIp) {
        ...
        return new ConnectionReport.Check("Gateway reachable", reachable,
                reachable ? "gateway responded" : "gateway did not respond",
                System.currentTimeMillis() - started);
    }

    private ConnectionReport.Check dnsCheck() {
        long started = System.currentTimeMillis();
        boolean dnsOk = resolvesAny(inBulk());
    ...
    private ConnectionReport.Check internetCheck() {
        ...
        boolean ok = ping.reachable("8.8.8.8", 3000)
                || (resolvesAny(List.of("www.google.com", "one.one.one.one")));
    ...
    private static boolean resolvesAny(List<String> hosts) {
